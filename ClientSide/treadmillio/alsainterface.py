...
def tukey_window(N, N_overlap=None):
    ...
    w[-N_overlap:] = 1/2 * \
        (1 - np.cos(2*np.pi*(N2-n[-N_overlap:])/(alpha * L)))
...
class Stimulus():
    def __init__(self, filename, data_buffer, channel, buffer_len, gain_db, window=None):
        ...
        # pre-initialized matrix (tensor actually) for easy summing
        ...
        if window is not None:
            _, self._new_window, self._old_window = tukey_window(
                self.buffer_len, window)
            self._new_window = np.transpose(
                np.tile(self._new_window, [self.n_channels, 1]))
            self._old_window = np.transpose(
                np.tile(self._old_window, [self.n_channels, 1]))
            self._old_gain = np.zeros(
                (buffer_len, self.n_channels))  # pre-allocate these
            self._new_gain = np.zeros(
                (buffer_len, self.n_channels))  # pre-allocate these
            self._gain_profile = np.zeros(
                (buffer_len, self.n_channels))  # pre-allocate these
    ...
    def get_nextbuf(self):
        ...
        if remainder > 0:
            ...
            self.data[:first, self.channel] = self.stimulus_buffer[self.curpos:(
                self.curpos+first)]
            self.data[first:, self.channel] = self.stimulus_buffer[:remainder]
            self.curpos = remainder
        else:
            self.data[:, self.channel] = self.stimulus_buffer[self.curpos:(
...
class ALSAPlaybackSystem():
    def __init__(self, dev_name, config, file_root, control_pipe, log_directory=None):
        ...
        self.xrun_filename = os.path.join(
            log_directory, 'alsa_playback_xruns.txt')

        self.control_pipe = control_pipe

        config['DeviceList'][dev_name] = normalize_output_device(
            config['DeviceList'][dev_name])
        ...
        for stimulus_name, stimulus in StimuliList.items():
            if stimulus['Type'] == 'Bundle':
                ...
                self.num_stimuli += len(glob.glob(os.path.join(file_root,
                                        root_dir, stimulus['Filename'])))
            else:
                self.num_stimuli += 1

        # data buffer for all data
        self.data_buf = np.zeros((buffer_size, num_channels, self.num_stimuli))
        k = 0
        for stimulus_name, stimulus in StimuliList.items():
            if stimulus_name in ILLEGAL_STIMULUS_NAMES:
                raise(ValueError(
                    '{} is an illegal name for a stimulus.'.format(stimulus_name)))
            ...
            if stimulus.get('Device', 'Default1') in channel_labels:
                ...
                if stimulus['Type'] == 'Bundle':
                    root_dir = stimulus.get('Directory', './')
                    filelist = sort_bundled_sounds(
                        glob.glob(os.path.join(file_root, root_dir, stimulus['Filename'])))
                    for i, filepath in enumerate(filelist):
                        self.stimuli['-'.join([stimulus_name, str(i)])] = Stimulus(filepath, self.data_buf[:, :, k],
                    ...
                    self.stimuli[stimulus_name] = Stimulus(
                        filename, self.data_buf[:, :, k], channel, buffer_size, gain, window=buffer_size)  # default to Hanning window!
                    k = k + 1
            else:
                print('When loading stimuli, {} not found in list of SpeakerChannels for device {}'.format(
                    stimulus.get('Device', 'Default1'), dev_name))
        ...
        # We'll always present stereo audio
        ...
        self.out_buf = np.zeros(
            (buffer_size, num_channels), dtype=dtype, order='C')
        ######
    ...
    def play(self):
        ...
        with open(self.xrun_filename, 'w') as xrun_logfile:
            ...
            while self.running:
                for _, stim in self.stimuli.items():
                    stim.get_nextbuf()
                self.out_buf[:] = self.data_buf.sum(axis=2).astype(
                    dtype=self.out_buf.dtype, order='C')
                res, xruns = self.adevice.write(self.out_buf)
                if xruns != 0:
                    print('xrun in playback [{}] at {}'.format(
                        xruns, time.monotonic()))
                    print('xrun in playback [{}] at {}'.format(
                        xruns, time.monotonic()), file=xrun_logfile)
...
class ALSARecordSystem():
    def __init__(self, dev_name, config, log_directory=None):
        ...
        if not log_directory:
            warnings.warn(
                "Recording microphone input to cwd because log file wasn't specified.")
            log_directory = os.getcwd()

        self.xrun_filename = os.path.join(
            log_directory, 'alsa_record_xruns.txt')
        ...
        # Open alsa device
        self.adevice = alsaaudio.PCM(
            alsaaudio.PCM_CAPTURE, alsaaudio.PCM_NORMAL, device=device)

        # We'll always record stereo audio TODO: support many channels
        self.adevice.setchannels(self.channels)
        self.adevice.setrate(self.fs)
        ...
        self.in_buf = np.zeros(
            (self.buffer_size*scale_factor, self.channels), dtype=dtype, order='C')

        self.adevice.enable_timestamps()

        self.logfilename = os.path.join(
            log_directory, '{}.wav.log'.format(dev_name))
        self.soundfilename = os.path.join(
            log_directory, '{}.wav'.format(dev_name))
    ...
    def record(self):
        ...
        with open(self.xrun_filename, 'w') as self.xrun_logfile, \
            ...
            print(
                'Timestamps for soundfile frames. Each record is [nsamps, time], where time is CLOCK_MONOTONIC.\n', file=self.logfile)
            while True:
                ...
                if nsamp < self.buffer_size:
                    ...
                    print('buffer underrun {}'.format(
                        t), file=self.xrun_logfile)


def normalize_output_device(config):
    config['BufferSize'] = config.get(
        'BufferSize', DEFAULT_OUTPUT_DEVICE['BufferSize'])
    config['DType'] = config.get('DType', DEFAULT_OUTPUT_DEVICE['DType'])
    config['Device'] = config.get(
        'HWDevice', DEFAULT_OUTPUT_DEVICE['HWDevice'])
    config['ChannelLabels'] = config.get(
        'ChannelLabels', DEFAULT_OUTPUT_DEVICE['ChannelLabels'])
    config['NChannels'] = config.get(
        'NChannels', DEFAULT_OUTPUT_DEVICE['NChannels'])
...
def normalize_input_device(config):
    config['BufferSize'] = config.get(
        'BufferSize', DEFAULT_INPUT_DEVICE['BufferSize'])
    config['DType'] = config.get('DType', DEFAULT_INPUT_DEVICE['DType'])
    config['Device'] = config.get('HWDevice', DEFAULT_INPUT_DEVICE['HWDevice'])
    config['SamplingRate'] = config.get(
        'SamplingRate', DEFAULT_INPUT_DEVICE['SamplingRate'])
    config['NChannels'] = config.get(
        'NChannels', DEFAULT_INPUT_DEVICE['NChannels'])
