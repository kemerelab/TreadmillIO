...
from .alsainterface import ALSAPlaybackSystem, ALSARecordSystem, NIPlaybackSystem
...
def run_NI_playback_process(device_name, config, file_dir, control_pipe, log_directory, status_queue):
    status_queue.put(1)
    try: 
        playback_system = NIPlaybackSystem(device_name, config, file_dir, control_pipe, log_directory)
    except Exception as e:
        status_queue.put(-1)
        status_queue.close()
        raise e
    ...
    try:
        ...
        print('Caught KeyboardInterrupt in NI playback process')
        playback_system.running = False # I don't think this does anything
    except Exception as e:
        raise e
    ...
    def __init__(self, sound_config, track_length=None, track_topology='Ring', log_directory=None, verbose=0):
        ...
        if 'DeviceList' in sound_config:
            ...
            for dev_name, dev in sound_config['DeviceList'].items():
                if dev['Type'] == 'Output':
                ...
                elif dev['Type'] == 'NationalInstrumentsOutput':
                    _playback_read_pipe, self.playback_pipe = Pipe()  # we'll write to p_master from _this_ process and the ALSA process will read from _playback_read_pipe
                    new_process = Process(target=run_NI_playback_process, args=(dev_name, sound_config, 
                                                sound_config['AudioFileDirectory'], _playback_read_pipe, log_directory, _startup_queue))
                    self._playback_processes.append(new_process)
                    new_process.daemon = True
                    new_process.start()     # Launch the sound process
                    ...
                    while(status < 2):
                        if (status < 0): # error in launching the playback process!
                            _startup_queue.close()
                            new_process.join()
                            raise(RuntimeError("An error occured in starting the NI playback process/object."))
                        status = _startup_queue.get()
        ...
        # TODO: Check to make sure that a playback process got started!!! 
...
def validate_sound_config(config):
    ...
    for dev_name, dev in config['DeviceList'].items():
        if dev['Type'] == 'Output' or dev['Type'] == 'NationalInstrumentsOutput':
